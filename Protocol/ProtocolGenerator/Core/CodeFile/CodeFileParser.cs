...
namespace ProtocolGenerator.Core
{
    public class CodeFileParser
    {
        ...
        public CodeFile ParserFile(FileInfo fileInfo)
        {
            ...
            if (lines == null || lines.Count == 0)
            {
//                Log.Error($"ParserFile {fileInfo.FullName} error : read file wrong .");
                return null;
            }
            ...
            Log.Error($"ParserFile {fileInfo.FullName} error : format file wrong .");
            return null;
        }
...
//            var importLines = new StringBuilder();
        ...
        private bool GetFormatEnumKeyLine(string line, CodeFile codeFile, out string formatLine)
        {
            ...
            {
                Log.Error($"please check the enum name {line}!");
                return false;
            }
        ...
        private bool GetFormatSyntaxLine(string line, CodeFile codeFile, out string formatLine)
        {
            ...
            if (formatArr.Length > 1)
            {
                ...
                {
                    Log.Error($"please check the syntax, this {syntax} is wrong!");
                    return false;
                }
            ...
            Log.Error("please check the syntax line!");
            return false;
        }
    }
}